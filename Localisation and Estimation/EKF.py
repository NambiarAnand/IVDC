...
for k in range(1, len(t)):  # start at 1 because we've set the initial prediciton
    ...
    # Add your code here !
    x_check[0] += v[k - 1] * np.cos(x_check[2]) * delta_t
    x_check[1] += v[k - 1] * np.sin(x_check[2]) * delta_t
    ...
    # Add your code here !
    F_km = np.array([[1, 0, -v[k - 1] * np.sin(x_check[2]) * delta_t],
                     [0, 1, v[k - 1] * np.cos(x_check[2]) * delta_t],
                     [0, 0, 1]], dtype=float)  
    ...
    for i in range(len(r[k])):
        if np.any(r[k][i] != 0):  # If any landmark measurement is available
            print('---------------------------')
            print(x_check)
